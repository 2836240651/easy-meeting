...
DST = SRC.with_name(SRC.stem + "_第5章表题字号字体修正_最终.docx")
...
def choose_title(section_title: str, prev_text: str, header: str, had_caption: bool, caption_title: str, is_cont: bool) -> str:
    ...
    if '接口路径' in header and '方法' in header:
        return sanitize_title("RESTful API接口测试结果续表" if is_cont else "RESTful API接口测试结果表")
    ...
    if is_cont and not caption_title:
        return sanitize_title("RESTful API接口测试结果续表")
...
for idx, b in enumerate(blocks):
    if isinstance(b, Paragraph):
        ...
        if is_blank_table(b):
            # Likely a layout spacer; keep it untouched and unnumbered.
            continue
