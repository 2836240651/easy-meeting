...
public class StringTools {
    ...
    public static final String encodebyMd5(String originString){return StringTools.isEmpty(originString) ? null: DigestUtils.md5Hex(originString);}
    public static final String getMeetingNoOrMettingId(){
        return StringTools.getRandomString(Constants.LENGTH_10);
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 10000; i++) {
            ...
            Integer abs = Math.abs(random.hashCode());
            Integer tableName = abs % 32 + 1;
            Integer count = HashMap.get(tableName);
            if (count == null) {
                HashMap.put(tableName,1);
            }else {
                HashMap.put(tableName,count + 1);
            }
        }
        HashMap.forEach((k,v)->{
